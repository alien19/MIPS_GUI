...
class Start_Window(QMainWindow):
    ...
    def InitWindow(asmbStart):
        ...
        #Creating a textbox
        asmbStart.textbox = TextEdit(asmbStart)
    ...
    def onClick1_assmbNow(asmbStart):
        ...
        dir = os.getcwd()
        p = os.system('"C:\Modeltech_pe_edu_10.4a\examples\prj_assembler.exe"') #os.path.join(dir,'prj_assembler.exe')
        ...
        for testCase in os.listdir(dirTC):
             if testCase.endswith(".txt"):
                 with open(os.path.join(dirTC,testCase)) as f:
                     ...
                     asmb.close()
                     p = os.system('"C:\Modeltech_pe_edu_10.4a\examples\prj_assembler.exe"')
    ...
    def onClick3_edu(asmbStart):
        textValue = asmbStart.textbox.toPlainText()
        asmbStart.textbox.clear()
        with open("assemblyCode.txt",'w') as e: #or 'a' if we want to append on the previous content of the file
            e.write(textValue)
...
class Edu(QFrame):
    ...
    def __init__(self,parent):
        ...
        self.label = QLabel(gotdata+"No   ",self)
        self.label.move(20,50)
        self.label.setFont(fedu)
        self.setPalette(pedu)
        self.label.adjustSize()

        self.label2 = QLabel('Instruction',self)
        self.label2.move(20,80)
        self.label2.setFont(fedu)
        self.setPalette(pedu)
    ...
    def paintEvent(self,event):
        ...
        try:
            gotdata = inst[self.i]
            self.label.setText(gotdata)
        except IndexError:
            gotdata = 'null'

        if self.counter<7 and gotdata != 'null':
            painter1.begin(self)
